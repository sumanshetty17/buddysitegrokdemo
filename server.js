...
const { PLANS, DISCOUNT_PERCENT, WATERMARK_ADDON, watermarkPricePaise, isWatermarkHidden, canUseHomepageBuilder } = require('./plans');
...
function wmFooter(site) {
  if (site && site._hideWatermark) return '';
  return '<footer>Made with BuddySite</footer>';
}
async function markWatermark(site) {
  if (!site) return site;
  const owner = await db.getUserById(site.user_id);
  site._hideWatermark = isWatermarkHidden(owner);
  return site;
}
...
app.post('/api/sites/:id/hero-slides', requireAuth, loadOwnedSite, async (req, res) => {
  const user = await db.getUserById(req.userId);
  const plan = PLANS[user.plan];
  if (!canUseHomepageBuilder(plan)) return res.status(403).json({ error: 'The homepage slider is available on the Grow plan and above.' });
...
app.post('/api/sites/:id/category-groups', requireAuth, loadOwnedSite, async (req, res) => {
  const user = await db.getUserById(req.userId);
  const plan = PLANS[user.plan];
  if (!canUseHomepageBuilder(plan)) return res.status(403).json({ error: 'Custom homepage sections are available on the Grow plan and above.' });
...
app.post('/api/sites/:id/sliding-sections', requireAuth, loadOwnedSite, async (req, res) => {
  const user = await db.getUserById(req.userId);
  const plan = PLANS[user.plan];
  if (!canUseHomepageBuilder(plan)) return res.status(403).json({ error: 'Sliding Products sections are available on the Grow plan and above.' });
  const { title, productIds } = req.body || {};
...
app.post('/api/sites/:id/payout-bank', requireAuth, loadOwnedSite, async (req, res) => {
  const result = await db.savePayoutBank(req.site.id, req.body || {});
  if (!result) return res.status(404).json({ error: 'Store not found.' });
  if (result.error) return res.status(400).json({ error: result.error });
  res.json({ payoutBank: result });
});

app.post('/api/sites/:id/withdraw', requireAuth, loadOwnedSite, async (req, res) => {
  const result = await db.requestWithdrawal(req.site.id, req.body && req.body.amount);
  if (!result) return res.status(404).json({ error: 'Store not found.' });
  if (result.error) return res.status(400).json({ error: result.error });
  res.json(result);
});
...
app.get('/api/billing/watermark', requireAuth, async (req, res) => {
  const user = await db.getUserById(req.userId);
  const purchases = (user && user.watermark_purchases) || 0;
  const first = purchases < 1;
  res.json({
...
app.post('/api/billing/watermark/demo-activate', requireAuth, async (req, res) => {
  const user = await db.applyWatermarkAddon(req.userId);
  if (!user) return res.status(404).json({ error: 'Account not found.' });
...
${socialBarHtml}
<footer>
  ${site._hideWatermark ? '' : `<p class="footer-tagline">This store was built with BuddySite — build your own free online store at BuddySite.</p>
...
app.get('/store/:slug/cart', async (req, res) => {
  const site = await markWatermark(await db.getSiteBySlug(req.params.slug));
...
app.get('/store/:slug/about', async (req, res) => {
  ...
  site._hideWatermark = isWatermarkHidden(user);
...
// ---------- Seller help bot ----------
const helpBot = require('./help-bot');
app.post('/api/help/chat', async (req, res) => {
  try {
    const message = (req.body && req.body.message) || '';
    const result = await helpBot.answer(message);
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message || 'Help bot failed.' });
  }
});
...
app.post('/api/help/feedback', async (req, res) => {
  try {
    ...
    const comment = String((req.body && req.body.comment) || '').slice(0, 1000);
    if (!rating || rating < 1 || rating > 5) return res.status(400).json({ error: 'Rating must be 1–5.' });
    const fs = require('fs');
    const path = require('path');
    const file = path.join(__dirname, 'help-feedback.json');
    let list = [];
    ...
    res.status(500).json({ error: e.message || 'Could not save feedback.' });
  }
});
