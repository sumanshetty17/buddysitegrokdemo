...
async function applyWatermarkAddon(userId) {
  ...
  const r = await query(
    `UPDATE users SET watermark_until = $1, watermark_purchases = $2 WHERE id = $3 RETURNING *`,
    [start.toISOString(), purchases + 1, Number(userId)]
  );
  return r.rows[0] || null;
}
...
async function getFinanceSummary(siteId) {
  ...
  const ledgerOut = ledger.map(r => {
    ...
    return {
      ...
      excludedAsCancelled: !!isCancelled,
...
async function savePayoutBank(siteId, { accountName, accountNumber, ifsc, upi }) {
  const site = await loadSite(siteId);
  if (!site) return null;
...
async function requestWithdrawal(siteId, amount) {
  ...
  const id = await nextCounter('withdrawal');
  const w = {
    id,
  ...
  await saveSiteData(siteId, site);
  const fresh = await getFinanceSummary(siteId);
  return { withdrawal: w, wallet: fresh.wallet };
}
...
async function initSchema() {
  ...
  await query(schema);
  await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS watermark_until TEXT`);
  await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS watermark_purchases INTEGER NOT NULL DEFAULT 0`);
}
