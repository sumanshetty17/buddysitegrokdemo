...
const PLANS = {
  ...
  starter: {
    ...
    upiCod: true,
    platformFeePercent: 2,
    blurb: 'Custom domain, WhatsApp, coupons and reviews. Watermark removable as an add-on.'
  },
  grow: {
    key: 'grow',
    name: 'Grow',
    ...
    blurb: 'Homepage slider, category sections, sliding products, plus analytics and shipping.'
  },
  pro: {
    key: 'pro',
    name: 'Pro',
...
// Paid watermark removal (all plans). First 6 months 50% off, then full price.
const WATERMARK_ADDON = {
  first_paise: 29900,     // ₹299 first 6 months
  regular_paise: 59900,   // ₹599 every later 6 months
  months: 6
};

function watermarkPricePaise(purchases) {
  return (Number(purchases) || 0) < 1 ? WATERMARK_ADDON.first_paise : WATERMARK_ADDON.regular_paise;
}

function isWatermarkHidden(user) {
  if (!user || !user.watermark_until) return false;
  return new Date(user.watermark_until).getTime() > Date.now();
}

function canUseHomepageBuilder(plan) {
  return !!(plan && plan.homepageBuilder);
}
...
module.exports = { PLANS, DISCOUNT_PERCENT, COMMISSION_RATES, commissionRateForPlan, WATERMARK_ADDON, watermarkPricePaise, isWatermarkHidden, canUseHomepageBuilder };
