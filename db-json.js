...
function empty() {
  return {
    ...
    nextCustomerId: 1, nextAddressId: 1, nextWithdrawalId: 1
  };
}
...
function createUser({ name, email, password_hash }) {
  const data = load();
  const user = { id: data.nextUserId++, name, email, password_hash, plan: null, plan_renews_at: null, paid_cycles: 0, watermark_until: null, watermark_purchases: 0, created_at: new Date().toISOString() };
...
function applyWatermarkAddon(userId) {
  const { WATERMARK_ADDON } = require('./plans');
  const data = load();
  const user = data.users.find(u => u.id === Number(userId));
  if (!user) return null;
  const purchases = user.watermark_purchases || 0;
  const start = user.watermark_until && new Date(user.watermark_until) > new Date()
    ? new Date(user.watermark_until)
    : new Date();
  start.setMonth(start.getMonth() + (WATERMARK_ADDON.months || 6));
  user.watermark_until = start.toISOString();
  user.watermark_purchases = purchases + 1;
  save(data);
  return user;
}
...
function getFinanceSummary(siteId) {
  ...
  const ledgerOut = ledger.map(r => {
    ...
    return { ...r, order, netCommission: net, orderRefundedAmount, excludedAsCancelled: !!isCancelled, paymentMethod: order ? order.paymentMethod : null };
  });

  const withdrawals = (site && site.withdrawals) || [];
  const walletSummary = wallet.computeWallet({ orders, ledger, withdrawals });
  ...
  return {
    ...
    wallet: {
      ...walletSummary,
      payoutBank: (site && site.payoutBank) || null,
      withdrawals: withdrawals.slice().reverse()
    }
  };
}

function savePayoutBank(siteId, { accountName, accountNumber, ifsc, upi }) {
  const data = load();
  const site = data.sites.find(s => s.id === Number(siteId));
  if (!site) return null;
  ...
  const upiId = String(upi || '').trim();
  if (!name || !number || !ifscCode) return { error: 'Enter account name, account number, and IFSC.' };
  if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifscCode)) return { error: 'Enter a valid IFSC code.' };
  site.payoutBank = { accountName: name, accountNumber: number, ifsc: ifscCode, upi: upiId, updated_at: new Date().toISOString() };
  save(data);
  return site.payoutBank;
}

function requestWithdrawal(siteId, amount) {
  const data = load();
  const site = data.sites.find(s => s.id === Number(siteId));
  if (!site) return null;
  if (!site.payoutBank || !site.payoutBank.accountNumber) {
    return { error: 'Add your bank details before withdrawing.' };
  }
  const summary = getFinanceSummary(siteId);
  const available = summary.wallet.availableToWithdraw;
  const rupees = amount == null || amount === '' ? available : Number(amount);
  if (!rupees || rupees <= 0) return { error: 'Enter a valid withdrawal amount.' };
  if (rupees > available + 0.005) {
    return { error: `You can withdraw up to ₹${available.toFixed(2)}. Online net minus COD commission already held.` };
  }
  if (!Array.isArray(site.withdrawals)) site.withdrawals = [];
  if (!data.nextWithdrawalId) data.nextWithdrawalId = 1;
  const w = {
    id: data.nextWithdrawalId++,
    amount: Math.round(rupees * 100) / 100,
    onlineCommissionHeld: summary.wallet.onlineCommission,
    codCommissionHeld: summary.wallet.codCommissionDue,
    status: 'requested',
    created_at: new Date().toISOString(),
    bank: { ...site.payoutBank }
  };
  site.withdrawals.push(w);
  save(data);
  return { withdrawal: w, wallet: getFinanceSummary(siteId).wallet };
}
...
module.exports = {
  createUser, getUserByEmail, getUserById, updateUserPlan, applyWatermarkAddon,
  ...
  savePayoutBank, requestWithdrawal,
